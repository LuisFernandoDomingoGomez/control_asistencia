...
class AsistenciaController extends Controller
{
    ...
    {
        $this->middleware('permission:ver-asistencia|importar-archivo-asistencia|crear-asistencia|editar-asistencia|borrar-asistencia|generar-informes-asistencia')->only('index');
        $this->middleware('permission:crear-asistencia', ['only' => ['create','store']]);
        $this->middleware('permission:importar-archivo-asistencia', ['only' => ['import']]);
        $this->middleware('permission:editar-asistencia', ['only' => ['edit','update']]);
        $this->middleware('permission:borrar-asistencia', ['only' => ['destroy']]);
        $this->middleware('permission:generar-informes-asistencias', ['only' => ['export','export2']]);
    }
    ...
    {
        Asistencia::truncate();
        return redirect()->route('asistencias.index')->with('success', '¡Todos los registros de asistencia han sido eliminados!');
    }
    ...
    {
        return Excel::download(new ConcentradoExport, 'concentrados.xlsx');
    }
